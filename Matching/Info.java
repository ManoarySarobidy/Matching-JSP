// ito no classe hisy an'ny information
package object;
import formulary.DropDown;
import principal.Function;

public class Info extends BddObject{
	...
	public Info(String idUser) throws Exception{
	...
	public void init(){
		...
		super.setPrefix("INF");
		super.setSeqName("getInf");
	}

	public Info getInfo() throws Exception{
		try{
			Info info = (Info)this.select(this.getIdUser()).get(0);
			return info;
		}catch (Exception e) {
			throw new Exception("Tsy manana info io olona io");
		}
	}

	public void setIdInfo( String idInf ) throws Exception{
		if( idInf == null ){
			throw new Exception("L'IdInfo est null");
		}
		this.idInfo = idInf;
	}
